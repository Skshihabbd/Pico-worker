...
async function run() {
  try {
    ...
    app.post("/jwt", async (req, res) => {
      console.log("shihab eta email",req.body);
      const user = req.body;
    ...
    app.get("/user", async (req, res) => {
      ...
      let query = {};
      if (req.query?.email) {
        console.log('this email ache anik',req.query.email)
    ...
    app.patch("/infoupdate", async (req, res) => {
      ...
      try {
        ...
        const result = await userData.findOneAndUpdate(query, document, {
    ...
    app.post("/create-payment-intent", async (req, res) => {
      ...
      const priceInCentConvert = parseInt(price) * 100;
      if (!price || !priceInCentConvert) return;
      // generate clientsecret
      const { client_secret } = await stripe.paymentIntents.create({
        amount: priceInCentConvert,
        currency: "usd",
